...
class IFB(BaseNeuron):
    def __init__(self, S=0.03e-2, V_reset=-0.060, V_th=-0.050, V_spike=0.020, g_leak=0.035e-3, E_leak=-0.075, C_m=2e-6, T_ref=0, V_h=-0.070, g_T=0.8e-3, E_T=0.120, tau_h_plus=100e-3, tau_h_minus=20e-3):
        super().__init__(V_reset, V_th, V_spike, (g_leak * S), E_leak, (C_m * S), T_ref)
    ...
    def run(self, I_stim, dt, V_0 = None):
        ...
        # Parameter h(0) = 0, since 1 - h is the fraction of inactivated channels
        self.h = np.zeros(np.shape(self.I_stim))
        ...
        for trial_idx in np.arange(np.shape(self.I_stim)[0]):
            ...
            for i in np.arange(1, np.size(self._t)):
                if self._t[i] - self._t_last_spike > self.T_ref:                
                    ...
                    # if self._V_m[trial_idx][i-1] >= self.V_h:
                    #     m_inf = 1
                    #     self.h[trial_idx][i] = self.h[trial_idx][i-1] - self.h[trial_idx][i-1]*dt/self.tau_h_minus
                    # else:
                    #     m_inf = 0
                    #     self.h[trial_idx][i] = self.h[trial_idx][i-1] + (1 - self.h[trial_idx][i-1])*dt/self.tau_h_plus

                    # I_leak = self.g_leak * (self._V_m[trial_idx][i-1] - self.E_leak)
                    # I_T = self.g_T * self.h[trial_idx][i] * (self._V_m[trial_idx][i-1] - self.E_T) * m_inf
                    # I_tot = self.I_stim[trial_idx][i] - I_leak - I_T
